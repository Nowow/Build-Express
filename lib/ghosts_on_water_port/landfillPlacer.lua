--This software is based Geom2D (https://github.com/dewiniaid/BlueprintExtensions/blob/master/geom2d.lua) Original Licencse: Geom2D_LICENCE.txt

---This Module is used to place ghost landfill under dummy entity ghosts
local landfillPlacer = {}
local Geom2D = require('external-lib/Geom2D')
...
local function getTilesInBoundingBox(entity)
    ...
    if entity.ghost_type == "offshore-pump" then
        local orignalName = getOriginalEntityName(entity.ghost_name)
...
landfillPlacer.placeGhostLandfill = function(dummyEntity, tiles)
    ...
    table.each(tilesUnderEntity, function(tile)
        ...
        local new_tile_ghost = surface.create_entity { name = "tile-ghost", position = tile.position, force = dummyEntity.force,
        raise_built = true ,inner_name = usedLandfillType }
        table.insert(placed_tile_ghosts, new_tile_ghost)
    end)
    return placed_tile_ghosts
end
